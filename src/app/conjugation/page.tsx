...
import { ConjugationPanelWrapper } from "./ConjugationPanelWrapper";
import { getBoxCount, getNextVerb } from "@/app/actions/verbs";
...
export default async function Learn() {
  ...
  const initVerbResult = await getNextVerb(userId, 0);
    ...
    <main className="flex-1 flex flex-col items-center justify-between px-56 pb-3">
        <div className="flex flex-1 w-full bg-[var(--color-neutral-lightest)]">
          <ConjugationPanelWrapper
            userId={userId}
            initBoxes={initBoxes}
            initVerb={initVerbResult.success && initVerbResult.verb ? initVerbResult.verb : undefined}
          />
        </div>
    </main>
  );
